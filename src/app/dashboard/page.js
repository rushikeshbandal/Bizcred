...
export default function Dashboard() {
  ...
  const loadDashboard = async () => {
    try {
      ...
      const userGrowthPercent = lastUsers === 0 ? 100 : ((currentUsers - lastUsers) / lastUsers) * 100;
      const balanceGrowthPercent = lastBalance === 0 ? 100 : ((currentBalance - lastBalance) / lastBalance) * 100;
      ...
      const txRes = await fetch("/api/transactions/all", {
        headers: { Authorization: "Bearer " + token },
      });
  ...
  return (
    <AdminShell pageTitle="📊 Admin Dashboard" pageSubtitle="Welcome back, Admin 👋">
      {/* STATS CARDS */}
      <div style={cardGrid}>
      ...
      <div style={tableCard}>
        <div style={tableHeader}>
          <h2>👥 Recent Users</h2>
          <button style={viewBtn} onClick={() => (location.href = "/users")}>
            View All
          </button>
        </div>
        ...
        <table style={table}>
          ...
          <tbody>
            {recentUsers.map((u, i) => (
              <tr key={i}>
                <td style={td}>{u.name || "No Name"}</td>
                <td style={td}>{u.email}</td>
                <td style={td}>₹{u.wallet?.balance || 0}</td>
                <td style={td}>
                  <span
                    style={{
                      padding: "6px 12px",
                      borderRadius: "20px",
                      background:
                        u.status === "active" ? "#dcfce7" : u.status === "blocked" ? "#fee2e2" : "#fef3c7",
                    }}
                  >
                    {u.status}
                  </span>
